...
#endif
uint8 PAN_ID_Success=0;
/*********************************************************************
 * STATUS STRINGS
 */
  const char PingStr[]         = "Ping Receive from";
  const char AssocCnfStr[]     = "Assoc Config";
  ...
  const char AssocRspFailStr[] = "Assoc Respond fail";
  const char AssocIndStr[]     = "Assoc Ind";
  const char AssocCnfFailStr[] = "Assoc Config fail";
...
#if (ZDO_COORDINATOR)
        uint16 nwk_data;
         osal_nv_read(ZCD_NV_APP_PANID_H,0,sizeof(nwk_data),&nwk_data);
#if defined(FT_PAN_ID_C_R_C)
          if((statusValue-nwk_data)==1)  //写协调同级路由标志并复位重启  //当前PID网络存在设置成同级路由模式重启
          {
            //HalUARTWrite(HAL_UART_PORT_0, "\r\nttyy",6);
            osal_nv_read(ZCD_NV_APP_C_R_C,0,sizeof(nwk_data),&nwk_data);
           // nwk_data=(nwk_data&0XF000)|0X0AAA; //0X0AAA; 如果是协调器  这里改成路由重启(不改变最高位==保持协调器模式)，如果是路由器 不会到这里
            nwk_data=0X0AAA; //0X0AAA; 如果是协调器  这里改成路由重启(不改变最高位==保持协调器模式)，如果是路由器 不会到这里
          ...
          }// if(((statusValue-nwk_data)>10)||(nwk_data==0)||(statusValue==0))
          else     //当前PID网络不存在创建
          {
#if(ZDO_COORDINATOR==2)  //ZIGBEE  AT命令模式 透传模式
          HalUARTWrite(HAL_UART_PORT_0, "\r\n+CONNECT\r\n",12); //协调器网络创建成功
...
#else   
          if((((statusValue>nwk_data)?(statusValue-nwk_data):(nwk_data-statusValue))>2)||(nwk_data==0)||(statusValue==0))
              {  osal_nv_item_init( ZCD_NV_APP_PANID_H,sizeof(statusValue), &statusValue ); 
                osal_nv_write( ZCD_NV_APP_PANID_H, 0, sizeof(statusValue), &statusValue );
                osal_nv_item_init( ZCD_NV_APP_PANID_L,sizeof(statusValue), &statusValue );
                osal_nv_write( ZCD_NV_APP_PANID_L, 0,sizeof(statusValue),&statusValue);
               // printf("%s","BBBB");
              } 
#endif   
#endif   
...
#if((SENSOR_TYPE =='G')||(SENSOR_TYPE =='T')||(SENSOR_TYPE =='Q')||(SENSOR_TYPE =='O')||(SENSOR_TYPE =='P')\
  ||(SENSOR_TYPE ==0X02)||(SENSOR_TYPE ==0X06)||(SENSOR_TYPE ==0X20)||(SENSOR_TYPE ==0X07)\
    ||(SENSOR_TYPE ==0X08)||(SENSOR_TYPE ==0X09)||(SENSOR_TYPE ==0X0A)||(SENSOR_TYPE ==0X0B)\
      ||(SENSOR_TYPE ==0X23)||(SENSOR_TYPE ==0X18)||(SENSOR_TYPE ==0X13)||(SENSOR_TYPE ==0X14)\
      ||(SENSOR_TYPE ==0XA3)||(SENSOR_TYPE ==0X0D)||(SENSOR_TYPE ==0X6B)||(SENSOR_TYPE ==0X5A)||(SENSOR_TYPE ==0X5B)\
        ||(SENSOR_TYPE ==0X25)||(SENSOR_TYPE ==0X26)||(SENSOR_TYPE ==0X27)||(SENSOR_TYPE ==0X6F)\
          ||(SENSOR_TYPE ==0X70)||(SENSOR_TYPE ==0X71)||(SENSOR_TYPE ==0X0E)||(SENSOR_TYPE ==0X73)||(SENSOR_TYPE ==0X74)\
            ||(SENSOR_TYPE ==0X86)||(SENSOR_TYPE ==0X87)||(SENSOR_TYPE ==0X79)||(SENSOR_TYPE ==0X7B)||(SENSOR_TYPE ==0X7C))
