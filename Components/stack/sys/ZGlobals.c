...
#if defined(FT_PAN_ID_C_R_C)
#if defined SENSOR_TYPE_Coord 
uint16 ZG_BUILD_COORDINATOR_TYPE;
uint16 ZG_BUILD_RTR_TYPE;
uint16 ZG_BUILD_ENDDEVICE_TYPE;
uint16 ZG_BUILD_RTRONLY_TYPE;
uint16 ZG_BUILD_JOINING_TYPE;
uint16 DEVICE_LOGICAL_TYPE;
uint16 ZSTACK_DEVICE_BUILD;
uint16 ZG_DEVICE_COORDINATOR_TYPE;
uint16 ZG_DEVICE_RTR_TYPE;
uint16 ZG_DEVICE_JOINING_TYPE;
#endif
#endif
...
{  
  ...
  uint16 nwk_data;
 osal_nv_read(ZCD_NV_APP_CHANLIST,0,sizeof(nwk_data),&nwk_data);
  if(nwk_data>>8==0xaa) 
  {
        zgDefaultChannelList = ((uint32)0X00000001<<(nwk_data&0x00ff));
       // printf("%s","RRRRRRRRRRRRRRRRRRRRRRRRRRR");
   }
  else
  {   nwk_data=0XAA00;nwk_data+=CHANLIST_C_R_E; //清除协调同级路由标志并
     ...
     zgDefaultChannelList = ((uint32)0X00000001<<CHANLIST_C_R_E);
  }
  
#if defined(FT_PAN_ID_C_R_C)
#if defined SENSOR_TYPE_Coord 
if (ZDO_COORDINATOR)
   ZSTACK_DEVICE_BUILD=(DEVICE_BUILD_COORDINATOR);
#if defined ( RTR_NWK )
else
...
#if (ZDO_COORDINATOR)
//uint16 nwk_data;
 osal_nv_read(ZCD_NV_APP_C_R_C,0,sizeof(nwk_data),&nwk_data);
 if((nwk_data&0XF000)==0X5000) //设置成了 路由器模式 
   { ZSTACK_DEVICE_BUILD=(DEVICE_BUILD_ROUTER); //同级路由设置
    //printf("%s","PPPPPPPPPPPPPPPPP");
   }
   else
   {
   if(nwk_data==0X0AAA) //切换到协调同级路由启动
   {    ZSTACK_DEVICE_BUILD=(DEVICE_BUILD_ROUTER); //同级路由设置
      // printf("%s","IIIIIIIIIIIIIIIIIIIIIIIIII");
         osal_nv_read(ZCD_NV_APP_C_R_C,0,sizeof(nwk_data),&nwk_data);
         nwk_data=0X0BBB; //0X1BBB; 如果是协调器  这里改成下次启动协调器，如果是路由器 不会到这里
...
#endif
ZG_BUILD_COORDINATOR_TYPE=(ZSTACK_DEVICE_BUILD & DEVICE_BUILD_COORDINATOR);
ZG_BUILD_RTR_TYPE =(ZSTACK_DEVICE_BUILD & (DEVICE_BUILD_COORDINATOR | DEVICE_BUILD_ROUTER));
ZG_BUILD_ENDDEVICE_TYPE=(ZSTACK_DEVICE_BUILD & DEVICE_BUILD_ENDDEVICE);
ZG_BUILD_RTRONLY_TYPE=(ZSTACK_DEVICE_BUILD == DEVICE_BUILD_ROUTER);
ZG_BUILD_JOINING_TYPE=(ZSTACK_DEVICE_BUILD & (DEVICE_BUILD_ROUTER | DEVICE_BUILD_ENDDEVICE));

if ( ZSTACK_DEVICE_BUILD == DEVICE_BUILD_COORDINATOR )
ZG_DEVICE_COORDINATOR_TYPE=1;
else
...
ZG_DEVICE_RTR_TYPE=1;
else
ZG_DEVICE_RTR_TYPE=((zgDeviceLogicalType == ZG_DEVICETYPE_COORDINATOR) || (zgDeviceLogicalType == ZG_DEVICETYPE_ROUTER));
if( ZG_BUILD_COORDINATOR_TYPE )DEVICE_LOGICAL_TYPE=ZG_DEVICETYPE_COORDINATOR;
else if( ZG_BUILD_RTR_TYPE )DEVICE_LOGICAL_TYPE=ZG_DEVICETYPE_ROUTER;
else if( ZG_BUILD_ENDDEVICE_TYPE )DEVICE_LOGICAL_TYPE=ZG_DEVICETYPE_ENDDEVICE;
zgDeviceLogicalType= DEVICE_LOGICAL_TYPE;
ZG_DEVICE_JOINING_TYPE=((zgDeviceLogicalType == ZG_DEVICETYPE_ROUTER) || (zgDeviceLogicalType == ZG_DEVICETYPE_ENDDEVICE));
#endif
#endif
}
