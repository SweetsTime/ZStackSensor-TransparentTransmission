...
{     
	...
	// Initialization for board related stuff such as LED+s
	HAL_BOARD_INIT();
...
static void zmain_vdd_check( void )
{
	...
	do{ while (!HalAdcCheckVdd(VDD_MIN_RUN));
	} while (--cnt);
}
...
static void zmain_cert_init(void)
{
	  ...
	  (osal_memcmp(certData, nullData, ZCL_KE_CA_PUBLIC_KEY_LEN)))
	{
		...
		HalFlashRead(HAL_FLASH_IEEE_PAGE, HAL_FLASH_CA_PUBLIC_KEY_OSET, 
					 certData, ZCL_KE_CA_PUBLIC_KEY_LEN);
		...
		if(!osal_memcmp(certData, nullData, ZCL_KE_CA_PUBLIC_KEY_LEN))
		{
			...
			HalFlashRead(HAL_FLASH_IEEE_PAGE, HAL_FLASH_IMPLICIT_CERT_OSET,	certData, ZCL_KE_IMPLICIT_CERTIFICATE_LEN);
			(void)osal_nv_write(ZCD_NV_IMPLICIT_CERTIFICATE, 0,	ZCL_KE_IMPLICIT_CERTIFICATE_LEN, certData);
			HalFlashRead(HAL_FLASH_IEEE_PAGE, HAL_FLASH_DEV_PRIVATE_KEY_OSET, certData,	ZCL_KE_DEVICE_PRIVATE_KEY_LEN);
...
static void zmain_dev_info(void)
{
	uint8 i;
	uint8 *xad;
	uint8 addr_buf[Z_EXTADDR_LEN*2+1];
	...
	for (i=0; i<Z_EXTADDR_LEN*2; xad--)
	{
		uint8 ch;
		ch = (*xad >> 4) & 0x0F;
		addr_buf[i++] = ch + (( ch < 10 ) ? '0' : '7');
		ch = *xad & 0x0F;
		addr_buf[i++] = ch + (( ch < 10 ) ? '0' : '7');
	}
	addr_buf[Z_EXTADDR_LEN*2] = '\0';

	
#ifdef SERIAL_DEBUG_SUPPORTED
	//printf("TexasInstruments\n");
	//printf("IEEE: %s\n", addr_buf);
	...
	printf("MAC-MT Coord\n");
#else
	printf("MAC-MT Device\n");
#endif // ZDO
#elif defined( MT_NWK_FUNC)
#if defined( ZDO_COORDINATOR)
	printf("NWK Coordinator\n");
#else
	printf("NWK Device\n");
#endif // ZDO
#endif // MT_FUNC
