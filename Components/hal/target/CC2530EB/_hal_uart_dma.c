...
static uint16 HalUARTclerDMA(uint16 len)
{
  uint16 cnt;
...
static uint16 HalUARTWriteDMA(uint8 *buf, uint16 len)
{
  ...
  {
#if((SENSOR_TYPE ==0X5A)||(SENSOR_TYPE ==0X5B)||(SENSOR_TYPE ==0X13)||(SENSOR_TYPE ==0X14)||(SENSOR_TYPE ==0X74))
  P1_0=1;//转换到发送数据模式
#endif
...
#if((SENSOR_TYPE ==0X5A)||(SENSOR_TYPE ==0X5B)||(SENSOR_TYPE ==0X13)||(SENSOR_TYPE ==0X14))
  halMcuWaitUs(3400);//9600
 // halMcuWaitUs(10000);//2400
  //添加延时为了可以发送485的最后2字节 此处针对于9600
  P1_0=0;//转换到接收数据模式
#endif
#if((SENSOR_TYPE ==0X74))
 // halMcuWaitUs(3400);//9600
  halMcuWaitUs(10000);//2400
  //添加延时为了可以发送485的最后2字节 此处针对于2400
  P1_0=0;//转换到接收数据模式
#endif
