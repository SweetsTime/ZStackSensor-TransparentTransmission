...
//#if (defined( SENSOR_TYPE_Coord )||defined( SENSOR_TYPE_POWER ))
//#else   
  
#if(SENSOR_TYPE ==0X16)
 if(LMOUT&&GenericApp_BX==2)
 {GenericApp_appIO++;GenericApp_BX=0;}
 if(!LMOUT)
   GenericApp_BX=2;
#endif
if(GenericAppRouterEB_nub<=1)
{ 
#if defined( SENSOR_TYPE_Coord )    
if((PAN_ID_Success!=2)&&( ZSTACK_DEVICE_BUILD==DEVICE_BUILD_ROUTER))
  {
HalLedSet ( HAL_LED_1, HAL_LED_MODE_TOGGLE );
  }
   GenericAppCoordEB(); //C 事件处理
#endif
GenericAppRouterEB_nub=1001;
#if((SENSOR_TYPE ==0X66)||(SENSOR_TYPE ==0X76))
GenericAppRouterEB_nub=11;
#endif
#if((SENSOR_TYPE ==0X73)||(SENSOR_TYPE =='G'))
GenericAppRouterEB_nub=2;
#endif
  //HalLedSet ( HAL_LED_1, HAL_LED_MODE_TOGGLE );
#if !defined( SENSOR_TYPE_Coord )
  GenericAppEndDeviceEB(); //R 传感器 事件处理
#endif
